...
function kratos_auto_post_link($content) {
  global $post;
  $content = preg_replace('/<\s*img\s+[^>]*?src\s*=\s*(\'|\")(.*?)\\1[^>]*?\/?\s*>/i', "<img src=\"$2\" title=\"".$post->post_title."\"  alt=\"".$post->post_title."\" />", $content);
  return $content;
  }
...
function kratos_keywords(){
        ...
        else{ echo trim(single_cat_title('',FALSE)); }
}
...
function kratos_blog_thumbnail_new() {
    ...
    $img_url = $img_url[0];
    if ( has_post_thumbnail() ) {
        echo '<a href="'.get_permalink().'"><img src="'.$img_url.'"  title="'.get_the_title().'" alt="'.get_the_title().'" /></a>';
    } else {
        $content = $post->post_content;
        $img_preg = "/<img (.*?)src=\"(.+?)\".*?>/";
        ...
        if(!empty($img_val)){
            echo '<a href="'.get_permalink().'"><img src="'.$img_val.'" title="'.get_the_title().'" alt="'.get_the_title().'"  /></a>';
        } else {
...
//给文章加内链
 ...
 $content .= '  title="'.get_the_title().'" alt="'.get_the_title().'" class="neilian-thumb"></a></div></div>';
 }
 wp_reset_postdata();
 return $content;
 }
